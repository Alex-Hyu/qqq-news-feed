...
st.set_page_config(page_title="ES结构位分析器", layout="wide")

st.title("📊 ES 日线结构位分析器")
st.markdown("""
...
# 文件上传
uploaded_file = st.file_uploader("上传ES日线CSV文件", type=['csv'])
...
if uploaded_file is not None:
    ...
    st.info("👆 请上传TradingView导出的ES日线CSV文件")
    
    st.markdown("""
    ### 如何导出数据
    1. 在TradingView打开ES日线图
    2. 确保时间框架选择 **1D (日线)**
